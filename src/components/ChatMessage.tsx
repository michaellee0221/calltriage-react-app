...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  return (
    <div className="flex items-start gap-2.5">
      ...
      <div className="flex flex-col w-full max-w-md items-start">
        <div className="flex items-center space-x-2 rtl:space-x-reverse">
          <span className="text-sm font-normal text-gray-50">{time}</span>
        </div>

        {type === "0" && (
          <div className="flex flex-col leading-1.5 p-3 border-gray-200 bg-gray-50 bg-opacity-30 rounded-e-xl rounded-es-xl w-fit max-w-[80%] sm:max-w-[75%] md:max-w-[70%] lg:max-w-[60%]">
            <p className="text-sm font-normal text-gray-50 break-words whitespace-pre-wrap">
              {message}
            </p>
          </div>
        )}
